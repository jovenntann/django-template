...
# Services
from domain.products.services.service_Category import get_category_by_id, delete_category_by_object, \
    update_category_by_object
...
class CategoriesIdAPIView(APIView):
    ...
    def get(request, category_id=None):
        ...
        category_object = get_category_by_id(category_id)
        if category_object is None:
            raise Http404
        category_serializer = ReadCategorySerializer(category_object)
    ...
    def delete(request, category_id=None):
        ...
        if category_object is None:
            raise Http404
        # Copy category_object so that we can return this data from our delete response
        response = {
            'operation': 'delete',
            'domain': 'products',
            'model': 'Category',
        ...
        delete_category_by_object(category_object)
    ...
    def put(request, category_id=None):
        ...
        if category_serializer.is_valid(raise_exception=True):
            update_category_by_object(
                category_object,
                category_serializer.validated_data.get('name', category_object.name),
                category_serializer.validated_data.get('description', category_object.description),
            )
